...
<div class="w-full">
    ...
    <section class="w-full bg-black flex justify-center items-center">
        <div class="w-full max-w-[1440px] flex flex-col gap-20 px-10 2xl:px-0">
            ...
            'assets/img/service-3.png') @slot('title', 'Desarrollo de
            aplicaciones') @endcomponent

            <div
                class="flex flex-wrap gap-8 items-start justify-between text-white mt-10"
            >
                @component('_components.card-service') @slot('title',
                'Aplicaciones a Medida:') @slot('description', 'En PDT, no solo
                creamos aplicaciones, construimos soluciones personalizadas para
                tus necesidades específicas. Desde concepto hasta desarrollo,
                cada línea de código está diseñada para potenciar tu visión.')
                @endcomponent @component('_components.card-service')
                @slot('title', 'Experiencias Interactivas:')
                @slot('description', 'Más allá de la funcionalidad, nos
                enfocamos en la interactividad. Desarrollamos aplicaciones que
                no solo resuelven problemas, sino que también crean experiencias
                inmersivas y memorables para tus usuarios.') @endcomponent
                @component('_components.card-service') @slot('title', 'Chatbots
                Inteligentes:') @slot('description', 'La comunicación 24/7 está
                a solo un clic de distancia. Nuestros chatbots inteligentes
                están diseñados para brindar respuestas rápidas y precisas,
                mejorando la experiencia del usuario y optimizando la
                eficiencia.') @endcomponent
            </div>
            ...
            >
                @component('_components.card-service') @slot('title',
                'Automatización Empresarial:') @slot('description', 'Optimizamos
                tus procesos con soluciones CRM y aplicaciones empresariales.
                Desde la gestión de clientes hasta la automatización de tareas,
                nuestro enfoque es mejorar la eficiencia y hacer crecer tu
                negocio.') @endcomponent @component('_components.card-service')
                @slot('title', 'Integración Perfecta:') @slot('description', 'Tu
                ecosistema digital merece coherencia. Desarrollamos aplicaciones
                y chatbots que se integran sin esfuerzo con tus sistemas
                existentes, asegurando una experiencia de usuario fluida y
                eficiente.') @endcomponent
                @component('_components.card-service') @slot('title', 'Análisis
                y Mejora Continua:') @slot('description', 'El desarrollo no se
                detiene en el lanzamiento. Implementamos análisis detallados
                para comprender el rendimiento de tus aplicaciones y chatbots,
                asegurando mejoras continuas para mantener tu negocio en la
                cima.') @endcomponent
            </div>
        </div>
    </section>
