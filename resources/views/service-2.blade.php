...
<div class="w-full">
    ...
    <section class="bg-black flex items-center justify-center py-10 px-10">
        ...
        >
            ...
            >
                ...
                <a href="{{ route('service-3') }}" class="btn" title="home">
                    <svg
                        width="47"
                        height="47"
                        viewBox="0 0 47 47"
                ...
                <a href="{{ route('service-4') }}" class="btn" title="category">
                    <svg
                        width="57"
                        height="47"
                        viewBox="0 0 57 47"
    ...
    <section class="w-full bg-black flex justify-center items-center">
        <div class="w-full max-w-[1440px] flex flex-col gap-20 px-10 2xl:px-0">
            ...
            'assets/img/service-2.png') @slot('title', 'Diseño gráfico')
            @endcomponent

            <div
                class="flex flex-wrap gap-8 items-start justify-between text-white mt-10"
            >
                @component('_components.card-service') @slot('title', 'Identidad
                Visual Impactante:') @slot('description', 'En PDT, creamos más
                que simples diseños; construimos identidades visuales
                impactantes. Desde logotipos hasta materiales de marketing,
                nuestro equipo de diseñadores gráficos da vida a tu marca de una
                manera que resuena y perdura.') @endcomponent
                @component('_components.card-service') @slot('title',
                'Creatividad sin Límites:') @slot('description', 'Imágenes que
                cuentan historias y capturan la esencia de tu mensaje. Nuestros
                diseñadores exploran límites creativos para darle a tu proyecto
                una apariencia única y memorable.') @endcomponent
                @component('_components.card-service') @slot('title', 'Diseño
                Responsivo:') @slot('description', 'La estética no se trata solo
                de lo visual, sino también de la adaptabilidad. Creamos diseños
                que se adaptan a diferentes plataformas y dispositivos,
                asegurando una experiencia coherente y atractiva en cualquier
                pantalla.') @endcomponent
            </div>
            ...
            >
                @component('_components.card-service') @slot('title',
                'Comunicación Visual Efectiva:') @slot('description', 'Cada
                diseño tiene un propósito. Nuestro enfoque se centra en
                comunicar tu mensaje de manera efectiva, utilizando elementos
                visuales que conectan y resuenan con tu audiencia.')
                @endcomponent @component('_components.card-service')
                @slot('title', 'Colaboración Creativa:') @slot('description',
                'Tu visión se une a nuestra creatividad. Colaboramos
                estrechamente contigo para entender tus metas y valores, creando
                diseños que no solo se ven bien, sino que también comunican la
                esencia de tu proyecto.') @endcomponent
                @component('_components.card-service') @slot('title', 'Evolución
                Estética Continua:') @slot('description', 'El diseño no es
                estático, y tampoco lo es nuestro enfoque. Estamos comprometidos
                con la evolución constante de tus elementos visuales para
                mantener tu presencia fresca y relevante en un mundo en
                constante cambio.') @endcomponent
            </div>
        </div>
    </section>
